     ...
     ${styles.paddingY} flex-col`}>
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        ...
        <div className="flex-[1.5] w-full flex flex-row
          ...
          {footerLinks.map((femi) => (
            <div key={femi.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]"> 
                <h4 className="font-poppins font-medium tert-[18px] leading-[27px] text-white">
                  {femi.title}
                ...
                <ul className="list-none mt-4">
                  {femi.links.map((link, index) => (
                <li key={link.name} 
                className={`font-poppins font-normal text-[16px] leading-[24px] text-white
                hover:text-secondary cursor-pointer ${index !==femi.links.length - 1 ? 'mb-4' : 'mb-0' }`}>
                     {link.name}  
                </li>
                  ))}   
                </ul>
